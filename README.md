# SitLess

SitLess 是一款 Windows 桌面端久坐提醒工具。应用常驻系统托盘，在工作日和用户配置的工作时间内运行，支持午休排除、上班确认、加班自动下班、提醒统计和自定义全屏提醒图片。

## 主要功能

- 按用户配置的上班、下班和午休时间控制提醒；v1 的上班时间必须早于同日下班时间。
- 支持“连续活跃”和“固定间隔”两种提醒模式。
- 支持轻柔、标准和强提醒三级节奏；标准模式会先显示系统通知和倒计时小窗，未处理时进入全屏休息提醒。
- 全屏提醒要求达到可配置的最短休息时长后才计为完成，并允许随时临时返回工作；提醒标语和休息操作文案均可自定义。
- 支持“稍后提醒”“跳过本次”“专注 30 分钟”和“今日不再提醒”。
- 下班后如果仍有键鼠活动，默认视为加班；达到单独配置的“加班无输入自动下班”时长后，会按最后活跃时间记录实际下班时间。
- 统计提醒、完成、跳过、稍后、中断、休息时长和最长久坐，提供日/周/月汇总及最近 30 天明细，并保存在本地 JSON 文件。
- 提供四张可选内置提醒图片；自定义提醒图片丢失时自动回退到内置图片，并在设置页明确提示。
- 今日诗词优先使用今日诗词接口，接口不可用时使用本地兜底内容。

## 提醒模式

### 连续活跃

连续活跃模式会根据键盘和鼠标输入判断用户是否持续使用电脑。持续活跃达到配置的阈值后触发提醒；如果无输入时间超过“无输入重置”配置，本轮计时会重新开始。选择该模式时，设置页只显示“连续活跃阈值”和“无输入重置”这类与当前模式相关的配置。

适合希望提醒更贴近真实使用状态的用户。

### 固定间隔

固定间隔模式从开始工作或上次处理提醒后开始计时，每隔配置的分钟数触发一次提醒。短暂空闲不会重置固定间隔。选择该模式时，设置页只显示“固定间隔”配置，不混入连续活跃模式的配置项。

适合希望提醒节奏稳定、规则简单的用户。

切换提醒模式后，当前提醒周期会重新开始，避免沿用旧模式的累计时间导致刚切换就触发提醒。

## 开发

安装依赖：

```powershell
npm install
```

预览 React 渲染层：

```powershell
npm run dev:renderer
```

构建并启动 Electron 桌面应用：

```powershell
npm start
```

如果需要用 Vite 开发服务运行 Electron，请分别在两个终端执行：

```powershell
npm run dev:renderer
npm run dev:electron
```

## 验证

```powershell
npm test
npm run build
npm run smoke:electron
```

## 打包

```powershell
npm run dist
```
